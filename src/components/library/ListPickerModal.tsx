...
export default function ListPickerModal({ userId, gameId, gameName, onClose }: Props) {
  ...
  useEffect(() => {
    async function fetchLists() {
      // Récupérer toutes les listes + vérifier si le jeu est déjà dedans
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-1.5">
              {lists.map(l => (
                <button
                  key={l.id}
                  onClick={() => toggleList(l.id, l.already_in)}
                ...
                >
                  {/* Checkbox */}
                  <div
                    className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 transition-all border-2"
                    style={{
                      background:   l.already_in ? 'var(--color-amber)' : 'transparent',
                      borderColor:  l.already_in ? 'var(--color-amber)' : 'var(--color-ink-subtle)',
                    }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-ink-subtle">
                      {l.game_count} jeux · {l.is_public ? 'Public' : 'Privé'}
                    </p>
                  </div>
        ...
        <div className="px-4 pb-4 pt-2 border-t border-surface dark:border-surface-dark">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-surface dark:bg-surface-dark text-ink dark:text-ink-dark rounded-[var(--radius-sm)] px-3 py-2 text-sm outline-none transition-all placeholder:text-ink-subtle focus:ring-2 focus:ring-amber/30"
            />
            <button
