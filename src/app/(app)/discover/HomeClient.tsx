...
import { Award, CalendarDays, Users, Flame, TrendingUp, Eye, Heart } from 'lucide-react'
...
/** Upgrade IGDB cover URL vers la résolution maximale pour le hero */
...
export default function HomeClient({ featuredGame, featuredRating, trendingGames, allGames, friendReviews, popularGames, userId }: Props) {
  ...
  useEffect(() => {
    fetch('/api/games/releases')
      ...
      .catch(() => setWeeklyGames([]))
      .finally(() => setLoadingWeekly(false))
  }, [])

  const visibleWeekly = weeklyGames.slice(0, 6)

  const [previewGames, setPreviewGames] = useState(() => allGames.slice(0, 16))
  useEffect(() => {
      ...
      {featuredGame && (
        <section className="relative h-[70vh] min-h-[500px] overflow-hidden">
          {featuredGame.cover_url && (
            <img
              src={heroImageUrl(featuredGame.cover_url) ?? ''}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {friendReviews.map((item: any) => (
                <div key={item.id} className="relative">
                  <div className="absolute -top-1 -right-1 z-10 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                    Ami
                  </div>
                  <ReviewCard
                    review={{
        ...
        {popularGames.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {friendReviews.filter((r: any) => (r.likes ?? 0) > 0).length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-sm font-semibold text-ink-subtle flex items-center gap-1.5">
                    <Heart className="w-4 h-4" /> Avis les plus aimés
                  </h3>
                  <div className="space-y-2">
                    {[...friendReviews]
                      .filter((r: any) => (r.likes ?? 0) > 0)
